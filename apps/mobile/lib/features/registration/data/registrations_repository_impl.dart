...
class RegistrationsRepositoryImpl implements RegistrationsRepository {
  ...
  Future<Either<Failure, List<RegistrationTicket>>> myRegistrations() async {
    try {
      final result = await _client.query(QueryOptions(document: gql(_qRegs)));
      ...
      return Right(
        list
            .map((e) => _mapTicket(e as Map<String, dynamic>))
            .toList(),
      );
    } catch (e, st) {
...
''';
  ...
  @override
  Future<Either<Failure, RegistrationTicket>> registerForEvent(String eventId) async {
    try {
  ...
  Future<Either<Failure, RegistrationTicket?>> myTicket(String eventId) async {
    try {
      final result = await _client.query(
        QueryOptions(document: gql(_qTicket), variables: {'eventId': eventId}),
      );
      if (result.hasException) {
        return Left(mapGraphQlException(result.exception!, StackTrace.current));
      }
